...
class FeedViewModel @Inject constructor(
        ...
        firestore.collection("feed")
            ...
            .addSnapshotListener { snap, err ->
                ...
                viewModelScope.launch {
                    ...
                    _posts.value = snap.documents.mapNotNull { doc -> doc.toPost() }
    ...
    private fun com.google.firebase.firestore.DocumentSnapshot.toPost(): Post? {
        ...
        val displayName = (d["displayName"] as? String)?.takeIf { it.isNotBlank() } ?: d["name"] as? String ?: ""
        ...
        val imageURL    = (d["imageURL"] as? String)?.takeIf { it.isNotBlank() } ?: d["imgUrl"] as? String ?: ""
    ...
    private suspend fun loadInteractions() {
        ...
        try {
            likedIds = firestore.collection("feedLikes").whereEqualTo("uid", uid)
                .get().await().documents.mapNotNull { it.getString("feedId") ?: it.getString("postId") }.toSet()
        ...
        viewModelScope.launch {
            try {
                ...
                if (nowLiked) {
                    ...
                    likeRef.set(mapOf("uid" to uid, "feedId" to post.id, "name" to myName, "photoURL" to myPhoto, "ts" to Timestamp.now())).await()
    ...
    fun toggleCommentLike(postId: String, comment: Comment) {
        ...
        _comments.value = _comments.value.map {
            if (it.id == comment.id) it.copy(isLikedByMe = nowLiked, likesCount = it.likesCount + if (nowLiked) 1 else -1) else it
        }
        viewModelScope.launch {
            try {
                val base = firestore.collection("feed").document(postId).collection("comments").document(comment.id)
                val likeRef = base.collection("likes").document(uid)
                if (nowLiked) {
                    likeRef.set(mapOf("uid" to uid, "ts" to Timestamp.now())).await()
                    base.update("likes", FieldValue.increment(1)).await()
                } else {
                    likeRef.delete().await()
                    base.update("likes", FieldValue.increment(-1)).await()
                }
    ...
    fun toggleSave(post: Post) {
        ...
        _posts.value = _posts.value.map { if (it.id == post.id) it.copy(isSavedByMe = nowSaved) else it }
        viewModelScope.launch {
            try {
    ...
    fun addComment(post: Post, text: String) {
        ...
        viewModelScope.launch {
            try {
                ...
                val myName  = userDoc.getString("displayName") ?: userDoc.getString("name") ?: auth.currentUser?.displayName ?: "Bikarhêner"
                val myPhoto = userDoc.getString("photoURL") ?: ""
                firestore.collection("feed").document(post.id).collection("comments").add(mapOf(
                    "uid" to uid, "name" to myName, "displayName" to myName, "photoURL" to myPhoto,
                    "text" to text, "likes" to 0, "replyTo" to "", "replyToCmtId" to "", "ts" to Timestamp.now(),
                )).await()
                firestore.collection("feed").document(post.id).update("cmtCount", FieldValue.increment(1)).await()
                _posts.value = _posts.value.map { if (it.id == post.id) it.copy(commentsCount = it.commentsCount + 1) else it }
    ...
    // ── Repost (DÜZELTİLDİ) — sadece referans, içerik kopyalanmaz ────────────
        ...
        viewModelScope.launch {
            try {
                ...
                firestore.collection("feed").add(mapOf(
                    // Repost yapan kişinin bilgileri
                    "uid" to uid, "name" to myName, "displayName" to myName,
                    "username" to myUser, "photoURL" to myPhoto,
                    // Bu kartın kendi içeriği yok
                    "text" to "", "imgUrl" to "", "imageURL" to "",
                    // Orijinal gönderiye referans
                    "repostType"        to "feed",
                    "repostId"          to post.id,
                    "repostUid"         to post.uid,
                    // Embed önizleme (tema uyumu)
    ...
    fun createPost(text: String, imageURL: String = "", quoteText: String = "", authorName: String = "", bookName: String = "") {
        ...
        viewModelScope.launch {
            try {
                ...
                firestore.collection("feed").add(mapOf(
                    "uid" to uid, "name" to myName, "displayName" to myName, "username" to myUser,
                    "photoURL" to myPhoto, "authorEmail" to myEmail, "text" to text,
                    "imgUrl" to imageURL, "imageURL" to imageURL,
                    "quote" to if (quoteText.isNotBlank()) mapOf("text" to quoteText, "book" to bookName, "author" to authorName) else null,
                    "quoteText" to quoteText, "authorName" to authorName, "bookName" to bookName,
                    "likes" to 0, "saves" to 0, "cmtCount" to 0, "reposts" to 0, "ts" to Timestamp.now(),
                )).await()
            } catch (e: Exception) { e.printStackTrace() }
        }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                val snap = firestore.collection("feed")
                    .orderBy("ts", Query.Direction.DESCENDING).startAfter(last).limit(PAGE_SIZE).get().await()
                if (snap.documents.isNotEmpty()) lastDoc = snap.documents.last()
                _hasMore.value = snap.documents.size >= PAGE_SIZE.toInt()
                val newPosts = snap.documents.mapNotNull { doc -> doc.toPost() }
    ...
    fun ensurePost(postId: String) {
        ...
        viewModelScope.launch {
            try {
                ...
                val p = doc.toPost() ?: return@launch
                _posts.value = _posts.value + p
            } catch (e: Exception) { e.printStackTrace() }
        }
    }
    ...
    private suspend fun sendNotif(toUid: String, type: String, title: String, sub: String = "", feedId: String = "") {
        try {
            ...
            firestore.collection("userNotifs").document(toUid).collection("msgs").add(mapOf(
                "fromUid" to uid, "fromName" to fromName, "fromPhoto" to fromPhoto,
                "type" to type, "feedId" to feedId, "postId" to feedId,
                "title" to title, "sub" to sub, "ico" to ico,
                "message" to title, "url" to "", "read" to false, "ts" to Timestamp.now(),
            )).await()
            firestore.collection("pushQueue").add(mapOf(
                "targetUid" to toUid, "title" to "Heftreng", "body" to title, "feedId" to feedId, "ts" to Timestamp.now(),
            )).await()
        } catch (e: Exception) { e.printStackTrace() }
    }
}
