    ...
    fun load(uid: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                // Takip durumu + sayılar paralel yükle
                kotlinx.coroutines.coroutineScope {
                    // Takip durumu (private kontrol için gerekli — önce bitmeli)
                    if (targetUid != myUid) {
                    ...
                    // Takipçi sayıları — users belgesindeki sayaçtan oku (Firestore count yok)
                    // users/{uid}.followersCount ve followingCount yazılıyorsa direkt kullan
                    val fCount = (d["followersCount"] as? Long)?.toInt()
                    val fgCount = (d["followingCount"] as? Long)?.toInt()
                    if (fCount != null) {
                        _followersCount.value = fCount
                        _followingCount.value = fgCount ?: 0
                    } else {
                        // Sayaç yoksa follows koleksiyonundan say (fallback — yavaş)
                        launch {
                            val snap = firestore.collection("follows")
                                .whereEqualTo("targetUid", targetUid).get().await()
                            _followersCount.value = snap.size()
                        }
                        launch {
                            val snap = firestore.collection("follows")
                                .whereEqualTo("fromUid", targetUid).get().await()
                            _followingCount.value = snap.size()
                        }
                    }
                }
                ...
                val rawProfilePosts = snap.documents.mapNotNull { doc ->
                    ...
                    // Like durumu: feedLikes N+1 sorgu — başlangıçta false, lazy güncellenir
                    val likeDoc: com.google.firebase.firestore.DocumentSnapshot? = null
