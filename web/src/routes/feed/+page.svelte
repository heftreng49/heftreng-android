      ...
      {#each filteredPosts as p (p.id)}
        ...
        <article class="card" onclick={() => window.location.href = '/post/' + p.id} role="button" tabindex="0">
              ...
              <div class="repost-embed" onclick={(e) => { e.stopPropagation(); window.location.href = '/post/' + p.repostId; }}>
                ...
                {#if p.repostAuthor}
                  <div class="repost-author">
                    <div class="repost-av">
                      {#if p.repostAuthorPhoto}
                        <img src={p.repostAuthorPhoto} alt={p.repostAuthor} />
                      {:else}
                        {p.repostAuthor[0].toUpperCase()}
                      {/if}
                    </div>
                ...
                {#if p.repostImg || p.serialCover}
                  <img src={p.repostImg || p.serialCover} alt="" class="repost-img" />
                {/if}
              </div>
            {/if}
          ...
          <div class="actions" onclick={(e) => e.stopPropagation()}>
            ...
            <button class="act-btn" onclick={() => window.location.href = '/post/' + p.id} aria-label="Yorum yap">
              ...
              {#if (p.cmtCount ?? p.commentsCount ?? 0) > 0}<span>{p.cmtCount ?? p.commentsCount}</span>{/if}
            </button>

            <button class="act-btn" class:reposted={p.isRepostedByMe} aria-label="Yeniden paylaş">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>
...
.avatar-img { width: 100%; height: 100%; border-radius: 50%; object-fit: cover; display: block; background: var(--surface-var); }
...
.quote-mark {
  ...
  left: 6px;
  font-size: 52px;
  color: color-mix(in srgb, #F59E0B 15%, transparent);
  font-weight: 900;
  line-height: 1;
  pointer-events: none;
...
.repost-title { font-size: 13px; font-weight: 600; color: var(--on-bg); line-height: 1.35; }
...
/* Aksiyonlar */
.actions { display: flex; align-items: center; gap: 0; margin-top: 4px; }
