<script lang="ts">
  ...
  import { register, signInWithGoogle }  from '$lib/services/auth.service';
  ...
  async function submit() {
    ...
    if (password.length < 6) {
      error = $lang === 'ku' ? 'Şîfre divê herî kêm 6 tîp be.' : 'Şifre en az 6 karakter olmalı.';
      return;
    }
    loading = true; error = '';
    try {
      ...
      error =
        e.code === 'auth/email-already-in-use'
          ? ($lang === 'ku' ? 'Ev e-name berê hatiye bikaranîn.' : 'Bu e-posta zaten kullanımda.')
          : e.code === 'auth/weak-password'
          ? ($lang === 'ku' ? 'Şîfre divê herî kêm 6 tîp be.' : 'Şifre en az 6 karakter olmalı.')
          : ($lang === 'ku' ? 'Qeydkirin têk çû.' : 'Kayıt başarısız.');
...
<div class="wrap">
  <div class="card">
    ...
    <p class="sub">{$lang === 'ku' ? 'Hesabek nû çêke' : 'Hesap oluştur'}</p>
    ...
    <div class="field">
      <label for="name">{s.fullName($lang)}</label>
      <input id="name" type="text" bind:value={displayName}
        placeholder={$lang === 'ku' ? 'Navê te' : 'Adın Soyadın'} />
    </div>
    <div class="field">
    ...
    <div class="field">
      ...
      <input id="pass" type="password" bind:value={password}
        placeholder={$lang === 'ku' ? 'Herî kêm 6 tîp' : 'En az 6 karakter'}
        onkeydown={(e) => e.key === 'Enter' && submit()} />
    </div>

    <button class="btn" onclick={submit} disabled={loading}>
      {loading
        ? ($lang === 'ku' ? 'Tê qeydkirin...' : 'Kayıt yapılıyor...')
        : s.register($lang)}
    </button>

    <p class="foot">{s.hasAccount($lang)} → <a href="/login">{s.login($lang)}</a></p>
  </div>
</div>
...
.btn { width:100%; padding:14px; background:var(--primary); color:#fff; border-radius:12px; font-size:16px; font-weight:700; cursor:pointer; border:none; font-family:inherit; margin-top:4px; transition:opacity .15s; }
