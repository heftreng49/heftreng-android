...
/** displayName'den benzersiz username üretir */
...
export async function signInWithGoogle() {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      ...
      username:       await generateUniqueUsername(cred.user.displayName ?? '', cred.user.uid),
      bio:            '',
      followersCount: 0,
      followingCount: 0,
      postsCount:     0,
...
export async function register(email: string, password: string, displayName: string) {
  ...
  await setDoc(doc(db, 'users', cred.user.uid), {
    ...
    email,
    photoURL:       '',
    username:       await generateUniqueUsername(displayName, cred.user.uid),
    bio:            '',
    followersCount: 0,
    followingCount: 0,
    postsCount:     0,
