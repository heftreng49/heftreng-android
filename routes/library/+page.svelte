...
<div class="page">
  ...
  {:else}
    ...
    {#if activeTab === 0}
      {#if quotes.length === 0}
        <p class="empty">Henüz alıntı yok.</p>
      {:else}
        <div class="quote-list">
          ...
          {#if quotesHasMore}
            <button class="load-more" onclick={loadMoreQuotes} disabled={quotesLoadingMore}>
              {quotesLoadingMore ? 'Yükleniyor…' : 'Daha fazla'}
            </button>
          {/if}
        </div>
      {/if}
    ...
    {:else if activeTab === 1}
      {#if reviews.length === 0}
        <p class="empty">Henüz inceleme yok.</p>
      {:else}
    ...
    {:else if activeTab === 3}
      {#if books.length === 0}
        <p class="empty">Henüz kitap yok.</p>
      {:else}
        <div class="book-grid">
...
<style>
  ...
  /* Yazarlar */
  .author-grid { display: flex; flex-direction: column; gap: 0; }
  .author-card { display: flex; align-items: center; gap: 14px; padding: 14px 16px;
                  border-bottom: 1px solid var(--divider); }
  .author-info { display: flex; flex-direction: column; gap: 2px; }
  .author-name   { font-weight: 700; font-size: 15px; color: var(--on-bg); }
  .author-nat    { font-size: 12px; color: var(--muted); }
  .author-counts { font-size: 12px; color: var(--muted); }

  /* Kitaplar */
  .book-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr));
  ...
  .book-cover-placeholder { width: 100%; aspect-ratio: 2/3; background: var(--surface-var);
  ...
  .load-more { width: 100%; padding: 14px; background: var(--surface-var); border-radius: 12px;
