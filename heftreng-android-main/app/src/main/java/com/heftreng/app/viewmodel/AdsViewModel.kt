...
class AdsViewModel @Inject constructor(
    ...
    private val nativeFeedPool = ArrayDeque<NativeAd>()
    private val FEED_POOL_SIZE = 5

    fun getNextFeedNative(): NativeAd? {
        val ad = nativeFeedPool.removeFirstOrNull()
        return ad
    }

    fun maintainNativePool(unitId: String) {
        repeat((FEED_POOL_SIZE - nativeFeedPool.size).coerceAtLeast(0)) {
            preloadNativeAd(unitId, NativeAdSlot.FEED)
        }
    }
    ...
    fun preloadNativeAd(unitId: String, slot: NativeAdSlot, adSize: String = "small") {
        ...
        AdLoader.Builder(appContext, unitId)
            .forNativeAd { nativeAd ->
                ...
                when (slot) {
                    NativeAdSlot.FEED -> { cachedNativeFeedAd?.destroy(); cachedNativeFeedAd = nativeAd
                        if (nativeFeedPool.size < FEED_POOL_SIZE) nativeFeedPool.add(nativeAd); _nativeFeedAd.value = nativeAd; nativeFeedRetryCount = 0 }
